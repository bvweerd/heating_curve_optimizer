...
# Constraint 3: Heat debt limit
if buffer_kwh >= -max_buffer_debt:  # Allows configurable heat debt
    dp[t].setdefault(off, {})
    # ... store feasible state
```

The non-negativity constraint has been relaxed to allow "heat debt" - enabling temporal load shifting by reducing heating during expensive hours and compensating during cheaper hours.
